...
def evaluate(args, processor, model, tokenizer, prefix=""):
    ...
    result = acc_f1_pea_spea(eval_task, preds, out_label_ids)
...
def main():
    ...
    # config = config_class.from_pretrained(args.config_name if args.config_name else args.model_name_or_path, num_labels=num_labels,proxies={'https':'http//proxy-shz.intel.com:911'},verify=False)
    # tokenizer = tokenizer_class.from_pretrained(args.tokenizer_name if args.tokenizer_name else args.model_name_or_path, do_lower_case=args.do_lower_case,proxies={'https':'http//proxy-shz.intel.com:911'},verify=False)
    # model = model_class.from_pretrained(args.model_name_or_path, from_tf=bool('.ckpt' in args.model_name_or_path), config=config,proxies={'https':'http//proxy-shz.intel.com:911'},verify=False)
...
if __name__ == "__main__":
    # train, test = train_test_split('nonghangzhidao_filter.csv')
    # train.to_csv('data_ran/train.csv',index=False)
    # test.to_csv('data_ran/test.csv',index=False)
    main()
